...
export default function LandingPage() {
  ...
  return (
    <WebsitePage className="hero-gradient mesh-gradient relative overflow-hidden">
      <motion.section
        ...
        <WebsiteHero>
          <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
            ...
            <div className="grid gap-5 sm:gap-6">
              <div className="grid grid-cols-3 gap-3 border-t border-[var(--color-border-subtle)] pt-3">
                {stats.map((item) => {
                  const Icon = item.icon;
                  return (
                    <div key={item.label} className="py-2 text-center border-l border-[var(--color-border-subtle)] first:border-l-0">
              ...
              <div className="space-y-2.5 border-t border-[var(--color-border-subtle)] pt-3">
                <p className="text-xs uppercase tracking-[0.12em] text-[var(--color-text-tertiary)]">Built For Real Groups</p>
                {[
      ...
      <WebsiteSection>
        ...
        >
          {features.map((feature) => {
            ...
            return (
              <article key={feature.title} className="py-4 border-t border-[var(--color-border-subtle)]">
                <div className="w-10 h-10 rounded-full bg-[rgba(220,20,60,0.18)] flex items-center justify-center mb-3">
        ...
        >
          <div className="pt-4 border-t border-[var(--color-border-subtle)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
              {flow.map((step, index) => (
                <div key={step.title} className="pl-4 border-l-2 border-[rgba(220,20,60,0.35)]">
