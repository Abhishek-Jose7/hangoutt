...
function run() {
  ...
  targetZones.forEach(zoneName => {
    ...
    dbPlaces.forEach(p => {
      ...
      if (hasAnyPattern(normalized, LOW_INTENT_CHAIN_PATTERNS)) return;
      if (hasAnyPattern(normalized, WEAK_OR_NON_HANGOUT_PATTERNS) && !strongSignal) return;
      ...
      if (category === 'RESTAURANT') {
        if (hasAnyPattern(normalized, GENERIC_WEAK_FOOD_PATTERNS) && !strongSignal) return;
        isWorthy = strongSignal || highlyReviewed || strongRated;
      } else if (category === 'PARK') {
        const scenicSignal = hasAnyPattern(normalized, ['promenade', 'beach', 'lake', 'garden', 'fort', 'national park', 'nature park', 'waterfront', 'viewpoint', 'central park', 'jio world garden']);
